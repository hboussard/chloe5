...
public class DistanceWeightedCountValueAndCoupleKernel extends Kernel {
	...
	public DistanceWeightedCountValueAndCoupleKernel(short[] values, float[] couples, int windowSize, short[] shape, float[] coeff, int width, int height, int dep, float[] imageIn, float[][] imageOut){
		this.setExplicit(true);
		this.setExecutionModeWithoutFallback(Kernel.EXECUTION_MODE.GPU);
		this.values = values;
		this.couples = couples;
		this.windowSize = windowSize;
		this.mid = windowSize / 2;
		this.shape = shape;
		this.coeff = coeff;
		this.width = width;
		this.height = height;
	...
	public void processPixel(int x, int y, int localY) {
		if(x%dep == 0 && y%dep == 0){
			...
			for(int i=0; i<imageOut[ind].length; i++){
				imageOut[ind][i] = 0f;
			}
