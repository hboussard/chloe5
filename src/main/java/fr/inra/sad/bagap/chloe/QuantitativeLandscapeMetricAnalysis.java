...
public class QuantitativeLandscapeMetricAnalysis extends LandscapeMetricAnalysis {
	...
	private int buffer; // default
	
	private float[][] outDatas;
	
	private final LandscapeMetricKernel kernel;
	
	public QuantitativeLandscapeMetricAnalysis(GridCoverage2D coverage, int roiX, int roiY, int roiWidth, int roiHeight, int bufferROIXMin, int bufferROIXMax, int bufferROIYMin, int bufferROIYMax, int displacement, LandscapeMetricKernel kernel, Counting counting) {
		super(coverage);
		this.roiX = roiX;
		this.roiY = roiY;
	...
	protected void doInit() {
		...
		// gestion des sorties
		outDatas = new float[((((roiWidth-1)/displacement)+1)*(((buffer-1)/displacement)+1))][6];
