...
public class ThresholdQuantitativeKernel extends Kernel {
	...
	public ThresholdQuantitativeKernel(int windowSize, short[] shape, int width, int height, int dep, float[] imageIn, double[][] imageOut, float noDataValue, int enveloppeInterne){
						...
						for (int dx = -mid; dx <= mid; dx += 1) {
							if(((x + dx) >= 0) && ((x + dx) < width)){
								...
								if(shape[ic] == 1){
									...
									if(v == noDataValue) {
										imageOut[ind][0] = imageOut[ind][0] + 1.0;
									}else{
										nb = nb + 1;
										sum = sum + v;
									}
