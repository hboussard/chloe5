...
import fr.inrae.act.bagap.chloe.kernel.sliding.SlidingFunctionalCountValueKernel;
...
public class TinySlidingLandscapeMetricAnalysisFactory {
	...
	public static SlidingLandscapeMetricAnalysis create(LandscapeMetricAnalysisBuilder builder, Coverage coverage)
		...
		System.out.println("single sliding");
		...
		if (MetricManager.hasOnlyQuantitativeMetric(metrics)) { // quantitative
			...
			if (metrics.size() == 1 && metrics.iterator().next().getName().equalsIgnoreCase("MD")) {
				...
				// analysis
				return new DoubleTinySlidingLandscapeMetricAnalysis(coverage, coverageBocage, roiX, roiY, roiWidth,
			...
			if (MetricManager.hasOnlyValueMetric(metrics)) {
				...
				if (builder.getWindowShapeType() == WindowShapeType.FUNCTIONAL) {
					...
					kernel = new SlidingFunctionalCountValueKernel(windowSize, displacement,
							Raster.getNoDataValue(), values, unfilters, inCellSize, function, dMax);
					...
					return new DoubleTinySlidingLandscapeMetricAnalysis(coverage, coverageFriction, roiX, roiY,
							roiWidth, roiHeight, bufferROIXMin, bufferROIXMax, bufferROIYMin, bufferROIYMax, nbValues,
							displacement, kernel, counting);
					...
					// analysis
					return new TinySlidingLandscapeMetricAnalysis(coverage, roiX, roiY, roiWidth, roiHeight,
							bufferROIXMin, bufferROIXMax, bufferROIYMin, bufferROIYMax, nbValues, displacement, kernel,
							counting);
				}
