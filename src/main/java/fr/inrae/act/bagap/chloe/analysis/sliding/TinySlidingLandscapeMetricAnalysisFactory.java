...
public class TinySlidingLandscapeMetricAnalysisFactory {
		...
		if(builder.getAnalysisType() == WindowAnalysisType.SLIDING){
			...
			for(int j=0; j<windowSize; j++){
				for(int i=0; i<windowSize; i++){
					...
					if(builder.getWindowDistanceType() == WindowDistanceType.WEIGHTED){
						// gestion des distances ponderees (gaussienne centree)
			...
			if(MetricManager.hasOnlyQuantitativeMetric(metrics)){ // quantitative
				...
				if(metrics.size() == 1 && metrics.iterator().next().getName().equalsIgnoreCase("MD")){
					...
					kernel = new DSBDistanceBocageKernel(windowSize, displacement, shape, coeffs, Raster.getNoDataValue(), unfilters);
					...
					// analysis
					return new DoubleTinySlidingLandscapeMetricAnalysis(coverage, coverageBocage, roiX, roiY, roiWidth, roiHeight, bufferROIXMin, bufferROIXMax, bufferROIYMin, bufferROIYMax, 7, displacement, kernel, counting);
					
				}else if(metrics.size() == 1 && metrics.iterator().next().getName().equalsIgnoreCase("bocage")){
					kernel = new DSBDetectionBocageKernel(windowSize, displacement, shape, coeffs, Raster.getNoDataValue(), unfilters);
				}else{
				...
				// analysis
				return new TinySlidingLandscapeMetricAnalysis(coverage, roiX, roiY, roiWidth, roiHeight, bufferROIXMin, bufferROIXMax, bufferROIYMin, bufferROIYMax, 7, displacement, kernel, counting);
				...
				if(MetricManager.hasOnlyValueMetric(metrics)){
					...
					if(builder.getWindowShapeType() == WindowShapeType.FUNCTIONAL){
						...
						kernel = new SlidingFunctionalDistanceWeightedCountValueKernel(windowSize, displacement, Raster.getNoDataValue(), values, unfilters, inCellSize, function, dMax);
						...
						return new DoubleTinySlidingLandscapeMetricAnalysis(coverage, coverageFriction, roiX, roiY, roiWidth, roiHeight, bufferROIXMin, bufferROIXMax, bufferROIYMin, bufferROIYMax, nbValues, displacement, kernel, counting);
						...
						// analysis
						return new TinySlidingLandscapeMetricAnalysis(coverage, roiX, roiY, roiWidth, roiHeight, bufferROIXMin, bufferROIXMax, bufferROIYMin, bufferROIYMax, nbValues, displacement, kernel, counting);
					}
				...
				SlidingLandscapeMetricKernel kernel = new SlidingPatchKernel(windowSize, displacement, shape, coeffs, Raster.getNoDataValue(), values, inCellSize, unfilters);
