...
public class EPPStandardizationOld extends EcoPaysageProcedure {

	private int scale;
	
	public EPPStandardizationOld(EcoPaysageManager manager) {
		super(manager);
	}
	
	public EPPStandardizationOld(EcoPaysageManager manager, int specificScale) {
		this(manager);
	...
	//	divisor = EcoPaysage.standardizeCompo(manager().compoFile(scale), count, manager().compoMetrics(), manager().importances(), scale);
		
	//	manager().setDivisor("composition_"+scale+"m", divisor);
